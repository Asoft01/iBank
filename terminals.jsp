...
<!--                                    <button class="btn btn-sm btn-flat px-0">Today</button>
        ...
        <div class="mt-4 d-flex justify-content-between">
            <div class="d-flex">
                ...
                <h3 class="font-weight-light ml-2" id="ct">
                    ${totalTerminal}
                </h3>
            </div>
        </div>
                    ...
                    <c:choose>
                        <c:when test="${listOfTerminal.length() > 0}">
                            <c:forEach var="i" begin="0" end="${listOfTerminal.length() - 1}">
                                <tr class="add-cursor view-single-terminal" id="tr_${listOfTerminal.getJSONObject(i).getString("terminalid")}">
                                    <td class="text-center">${i + 1}</td>
                                    <td class="t_id">${listOfTerminal.getJSONObject(i).getString("terminalid")}</td>
                                    <td class="m_id">${listOfTerminal.getJSONObject(i).getString("merchantid")}</td>
                                    <td class="m_cat_code">${listOfTerminal.getJSONObject(i).getString("merchant_category_code")}</td>
                                    <td class="m_c_code">${listOfTerminal.getJSONObject(i).getString("currency")}</td>
                                    <td class="m_location" hidden>${listOfTerminal.getJSONObject(i).getString("merchant_location_name_address")}</td>
                                    <td class="m_creation_date">${listOfTerminal.getJSONObject(i).getString("creationdate")}</td>
                                </tr>
                            </c:forEach>
                        </c:when>
                    </c:choose>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $("#btn_request_terminal_delete").click(function(){
                ...
                $.ajax({
                    ...
                    url: '${pageContext.request.contextPath}/requestterminaldelete',
        ...
        //                    }]
                ...
                $("#modal_terminal_id").html(selectedTID);
...
//                    }]
            ...
            $(".dataTables_filter input").attr("placeholder", "Search by terminal id, merchant id...");
            $(".dataTables_filter input").css({"width": "300px"});
            $(".tbl_merchants .dataTables_empty").html("No terminal is available");
            $(".dataTables_filter input").focusin(function(){
                $(this).addClass("border-app-bottom");
            });
