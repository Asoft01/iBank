    ...
    <div class="row">
        <div id="" class="col-md-12 effect tablediv" style="padding-top: 30px;">
            ...
            <div class="card mx-3 px-3 py-2 mt-5" style="top: -48px;">
                ...
                <div class="mt-3">
                    <table id="" class="table table-hover table-sm tbl_loan tbl_payroll_download">
                                ...
                                <c:forEach var="i" begin="0" end="${loanrequest.length() - 1}">
                                    ...
                                    <c:choose>
                                        <c:when test="${loanrequest.getJSONObject(i).getString('status') eq 0}">
                                            <td><a class="btn btn-danger btn-sm approve_btn" id="approveLoanRequest_${loanrequest.getJSONObject(i).getString("id")}"  data-approve="${loanrequest.getJSONObject(i).getString("id")}"><span>Disapproved</span></a></td>
                                        </c:when>
                                        <c:otherwise>
                                            <td><a class="btn btn-danger btn-success  btn-sm disapprove_btn" id="disapproveLeaveRequest_${leaverequest.getJSONObject(i).getString("id")}"  data-disapprove="${loanrequest.getJSONObject(i).getString("id")}"><span>Approved</span></a></td>
                                        </c:otherwise>
                                    </c:choose>
...
<div class="modal fade" id="requestLoan" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                ...
                <c:choose>
                    ...
                    <c:otherwise>
                        <select class="browser-default custom-select select-employee mb-2" id="select-employee">
                            ...
                            <c:choose>
                                <c:when test="${employeeDetail.length() > 0}">
                                    <c:forEach begin="0" end="${EmployeeLength -1}" var="i">
                                        <option id='fname' data-lastname='${employeeDetail.getJSONObject(i).get("lastname")}' data-grade='${employeeDetail.getJSONObject(i).get("grade")}' data-department='${employeeDetail.getJSONObject(i).get("department")}' data-designation='${employeeDetail.getJSONObject(i).get("designation")}' data-location='${employeeDetail.getJSONObject(i).get("location")}' data-division='${employeeDetail.getJSONObject(i).get("division")}'  data-region='${employeeDetail.getJSONObject(i).get("region")}' value="${employeeDetail.getJSONObject(i).get("firstname")}">${employeeDetail.getJSONObject(i).getString("firstname")}</option>
                                    </c:forEach>
                                </c:when>
                                <c:otherwise>
                        ...
                        <div class="md-form md-b-outline-app mb-4">
                            ...
                            <input type="text" id="lname" class="form-control" readonly placeholder="Last Name of Employee">
                    ...
                    <c:when test="${session.getString('usertype') eq 'Employee'}">
                        <div class="md-form md-b-outline-app mb-4">
                            <i class="fas fa-barcode prefix text-app-main"></i>
                                <!--<input type="hidden" id="ccode" class="form-control" value="${companyCode}">-->
                            <input type="text" id="fname" value="${employeeDetails.getJSONObject(0).getString("firstname")}" class="form-control" placeholder="First name of employee" readonly>
                        ...
                        <div class="md-form md-b-outline-app mb-4">
                            <i class="fas fa-signature prefix text-app-main"></i>
                            <input type="hidden" id="ccode" class="form-control" value="${companyCode}">
                            <input type="text" id="lname" value="${employeeDetails.getJSONObject(0).getString("lastname")}" class="form-control" placeholder="Last name of Employee" readonly>
                            <label for="lastName"></label>
                        </div>
    ...
    <script>
        ...
        $('#btn_request_loan').click(function (e) {
            ...
            let spinner = `<i class="fa fa-spinner fa-spin" aria-hidden="true" style="font-size:50px;"></i>`;
        ...
        $(".approve_btn").click(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response === "00") {
                        $("#approveLoanRequest_" + id).html("Approved").addClass("btn-success");

                    } else {
                        $("#approveLoanRequest_" + id).html("Failed");
        ...
        $("div.pime-grid-filter").html('<div class="d-flex mr-4"><button class="btn bg orange darken-4 btn-sm mr-auto text-white" data-toggle="modal" data-toggle="modal" data-target="#requestLoan" id="submitRequest" type="button"> <div id="spinner"></div> Request Loan </button><p class="ml-3 mr-1">Total Records:' + "${totalClient}" + '</p> </div>');
