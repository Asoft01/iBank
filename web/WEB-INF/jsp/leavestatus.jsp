        ...
        <div id="" class="col-md-12 effect tablediv" style="padding-top: 30px;">
            ...
            <div class="card mx-3 px-3 py-2 mt-5" style="top: -48px;">
                <div class="card mx-1 warning-color white-text px-2">
                    <div class="d-flex align-items-center"><h5 class="bg">Leave Status</h5>
                          <!--<h1>${companyCode}</h1>-->
                ...
                <div class="mt-3">
                    <table id="" class="table table-hover table-sm tbl_leave_status tbl_leave_status_download">
                        ...
                        <tbody>
                        <c:choose>
                            <c:when test="${leavestatus.length() > 0}">
                                <c:forEach var="i" begin="0" end="${leavestatus.length() - 1}">
                                    <tr class="add-cursor view-single-merchant tr_${i}" id="">
                                        <td class="text-center">${i + 1}</td>
                                        <td class="m_id">${thisarray.getJSONObject(i).getString("firstname")}</td>
                                        <td class="m_id">${thisarray.getJSONObject(i).getString("lastname")}</td>
                                        <td class="m_id">${thisarray.getJSONObject(i).getString("leaveTaken")}</td>
                                        <td class="m_id">${thisarray.getJSONObject(i).getString("leaveOutstanding")}</td>
                                        <td class="d-none">${thisarray.getJSONObject(i).getString("id")}</td>
                                    </tr>
                                </c:forEach>
                            </c:when>
                        </c:choose>
